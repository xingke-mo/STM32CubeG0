...
void DEMO_PostMMIMessage( uint32_t EventVal )
{
    ...
    if( joyevent == 0 )
    {
        if( osMessagePut( LCDMsgBox, event, 0 ) != osErrorOS )
        {
            joyevent = 1;
        }
    }
...
static void Display_sinkcapa_menu( uint8_t PortNum )
{
    ...
    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_FRS_SUPPORT_Msk )
    {
        str[3] = '*';
    }

    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_DRD_SUPPORT_Msk )
    {
        str[8] = '*';
    }

    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_USBCOMM_Msk )
    {
        str[13] = '*';
    }

    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_EXT_POWER_Msk )
    {
        str[18] = '*';
    }

    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_HIGHERCAPAB_Msk )
    {
        str[24] = '*';
    }

    if( DPM_Ports[PortNum].DPM_ListOfRcvSNKPDO[PortNum] & USBPD_PDO_SNK_FIXED_DRP_SUPPORT_Msk )
    {
        str[29] = '*';
    }
...
static void Display_command_menu( uint8_t PortNum )
{
    ...
    {
        _config = COMMANDROLE_SRC;
    }
    else
    {
        _config = COMMANDROLE_SNK;
    }

    if( USBPD_TRUE == DPM_Settings[PortNum].PE_RoleSwap )
    {
        _config |= COMMANDROLE_DRP;
    }

    if( USBPD_TRUE == DPM_USER_Settings[PortNum].PE_VconnSwap )
    {
        _config |= COMMANDROLE_VCONN;
    }
