...
static void                USBPD_VDM_UserCallback( MUX_TypeCMuxIdTypeDef TypeCMuxId, MUX_HPDStateTypeDef   HPDState );
    ...
    else
    {
        ...
        if( USBPD_PORTDATAROLE_DFP == DPM_Params[PortNum].PE_DataRole )
        {
            ...
            case SVDM_DP_CONFIG :
                ...
                if( CC1 == DPM_Params[USBPD_PORT_0].ActiveCCIs )
                {
...
void USBPD_VDM_UserReset( uint8_t PortNum )
{
    ...
    if( USBPD_PORT_1 == PortNum )
    {
        if( USBPD_TRUE == DPM_Params[USBPD_PORT_1].PE_IsConnected )
        {
            BSP_MUX_SetEQGain( TYPE_C_MUX_2, MUX_2_EQGAIN_MAX );
            BSP_MUX_Enable( TYPE_C_MUX_2 );
        }
        else
        {
            BSP_MUX_SetDPPinAssignment( TYPE_C_MUX_2, PLUG_ORIENTATION_FLIPPED, USB_ONLY_PIN_ASSIGNMMENT );
        }
    }
