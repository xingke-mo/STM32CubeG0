...
static uint8_t RecordInit( void )
{
    ...
    /* Register TIM MSP initialization callback function */
    HAL_TIM_RegisterCallback( &htim, HAL_TIM_BASE_MSPINIT_CB_ID, AppAudio_TIM_MspInit );
    HAL_TIM_RegisterCallback( &htim, HAL_TIM_BASE_MSPDEINIT_CB_ID, AppAudio_TIM_MspDeInit );
...
static uint8_t RecordDeInit( void )
{
    ...
    /* Un-register TIM MSP initialization callback function */
    HAL_TIM_UnRegisterCallback( &htim, HAL_TIM_BASE_MSPINIT_CB_ID );
    HAL_TIM_UnRegisterCallback( &htim, HAL_TIM_BASE_MSPDEINIT_CB_ID );
...
static uint8_t RecordStart( void )
{
    ...
    if( HAL_ADC_Start_DMA( &hadc1, ( uint32_t * )BufferSelect, BUFFER_SIZE_WORD ) != HAL_OK )
    {
        ret++;
    }
...
static uint8_t CopyToSD( void )
{
    ...
    {
        if( HAL_ADC_Start_DMA( &hadc1, ( uint32_t * )Buffer2, BUFFER_SIZE_WORD ) != HAL_OK )
        {
            ret++;
        }
    ...
    {
        if( HAL_ADC_Start_DMA( &hadc1, ( uint32_t * )Buffer1, BUFFER_SIZE_WORD ) != HAL_OK )
        {
            ret++;
        }
