...
KMODULE_RETURN _CalendarConfig( void )
{
    ...
    CharHeight = ( ( ( sFONT * )BSP_LCD_GetFont() )->Height );
    CharWidth = ( ( ( sFONT * )BSP_LCD_GetFont() )->Width );
...
static void Calendar_TimeDisplay( RTC_TimeTypeDef *time )
{
    ...
    BSP_LCD_DisplayStringAtLine( TIME_LINE_NB, ( uint8_t * ) showtime );
}
...
static void Calendar_TimePreAdjust( void )
{
    ...
    HAL_RTC_GetTime( &hrtc, &timecurrent, RTC_FORMAT_BCD );
    /* Get the RTC current Date */
    HAL_RTC_GetDate( &hrtc, &dateset, RTC_FORMAT_BCD );
...
static void Calendar_TimeShow( void )
{
    ...
    uint32_t xpos = ( BSP_LCD_GetXSize() - 128 ) >> 1;
    RTC_TimeTypeDef time = {0};
    RTC_DateTypeDef date = {0};
...
static void Calendar_DateDisplay( uint8_t Year, uint8_t Month, uint8_t Day )
{
    ...
    BSP_LCD_DrawRect( daycolumn, dayline, CharWidth * 2, CharHeight - 2 );

    /* Set the Back Color */
    BSP_LCD_SetBackColor( LCD_COLOR_ST_PINK );
    BSP_LCD_SetTextColor( LCD_COLOR_ST_PINK );
    BSP_LCD_FillRect( 0, 0, BSP_LCD_GetXSize(), Font20.Height );
    BSP_LCD_FillRect( 0, LINE( 1 ), BSP_LCD_GetXSize(), Font20.Height );
    /* Set the Text Color */
    BSP_LCD_SetTextColor( LCD_COLOR_WHITE );

    sprintf( ( char * )linedisplay, "    %s  20%02d     ", MonthNames[Month - 1], Year );
    BSP_LCD_DisplayStringAtLine( 0, ( uint8_t * )linedisplay );
    ...
    BSP_LCD_DisplayStringAtLine( 1, ( uint8_t * )linedisplay );

    /* Set the Back Color */
    BSP_LCD_SetBackColor( LCD_COLOR_ST_GREEN_LIGHT );
    BSP_LCD_SetTextColor( LCD_COLOR_ST_GREEN_LIGHT );
    BSP_LCD_FillRect( 0, LINE( 2 ), BSP_LCD_GetXSize(), Font20.Height );
    BSP_LCD_SetTextColor( LCD_COLOR_WHITE );
    BSP_LCD_DisplayStringAtLine( 2, ( uint8_t * ) " Mo Tu We Th Fr Sa Su   " );
    ...
    {
        /* display first space */
        BSP_LCD_DisplayChar( mcolumn, mline, ' ' );
        mcolumn += CharWidth;
        ...
        if( month / 10 )
        {
            BSP_LCD_DisplayChar( mcolumn, mline, ( ( month / 10 ) + 0x30 ) );
        }
        else
        {
            BSP_LCD_DisplayChar( mcolumn, mline, ' ' );
...
static void Calendar_DatePreAdjust( void )
{
    ...
    /* Get the RTC current Time */
    HAL_RTC_GetTime( &hrtc, &timeset, RTC_FORMAT_BCD );
    /* Get the RTC current Date */
    HAL_RTC_GetDate( &hrtc, &datecurrent, RTC_FORMAT_BCD );
...
static void Calendar_DateShow( void )
{
    ...
    while( UserAction == 0x00u )
    {
        ...
        BSP_LCD_DisplayStringAtLine( DATE_LINE_NB, ( uint8_t * ) showdate );
...
static void Calendar_TimeRegulate( RTC_TimeTypeDef *TimeInput, RTC_TimeTypeDef *TimeOutput )
{
    ...
    charwidth = BSP_LCD_GetFont()->Width;
    ...
    /* Display new value */
    BSP_LCD_DisplayStringAt( rowarray[index], LINE( TIME_LINE_NB ), ( uint8_t * )display, LEFT_MODE );
    ...
    while( UserEvent != JOY_SEL )
    {
        if( ( UserEvent == JOY_UP ) || ( UserEvent == JOY_DOWN ) )
        {
            ...
            do
            {
                ...
                /* Let some time to get long key press */
                HAL_Delay( 50 + ( speed * 10 ) );

                if( speed > 0 )
                {
                    speed--;
                }
            } while( BSP_JOY_GetState() == UserEvent );
...
static void Calendar_DateRegulate( RTC_DateTypeDef *DateInput, RTC_DateTypeDef *DateOutput )
{
    ...
    /* Regulate year */
    BSP_LCD_SetTextColor( LCD_COLOR_ST_PINK );
    BSP_LCD_DisplayStringAtLine( 9, ( uint8_t * ) "UP/DOWN: Select Year" );
    BSP_LCD_DisplayStringAtLine( 10, ( uint8_t * ) "SEL: to set  " );
    ...
    BSP_LCD_DisplayStringAtLine( 9, ( uint8_t * ) "UP/DOWN: Select Month " );
    BSP_LCD_DisplayStringAtLine( 10, ( uint8_t * ) "SEL: to set  " );
    ...
    BSP_LCD_ClearStringLine( 9 );
    BSP_LCD_ClearStringLine( 10 );
    BSP_LCD_DisplayStringAtLine( 9, ( uint8_t * ) "UP/DOWN/LEFT/RIGHT:" );
    BSP_LCD_DisplayStringAtLine( 10, ( uint8_t * ) "Select Day " );
    BSP_LCD_DisplayStringAtLine( 11, ( uint8_t * ) "SEL: to set  " );
...
static void Calendar_AlarmDisable( void )
{
    ...
    /* Display information */
    BSP_LCD_DisplayStringAtLine( TIME_LINE_NB, ( uint8_t * )"Alarm is disabled" );
    HAL_Delay( 500 );
}


/**
  * @brief  Alarm A Irq Handler makes led toggling
  * @param  None
  * @retval None
  */
