...
static CCxPin_TypeDef Check_cc_attachement( uint32_t CC1Voltage, uint32_t CC2Voltage )
{
    ...
    if( ( CC2Voltage > 350 ) && ( CC2Voltage < 2500 ) )
    {
        val = CC2;
    }
    ...
    {
        if( ( CC1Voltage > 650 ) && ( CC1Voltage < 2500 ) )
        {
            val = CC1;
        }
    }
    ...
    {
        if( ( CC1Voltage > 350 ) && ( CC1Voltage < 2500 ) )
        {
            val = CC1;
        }
    }
...
static DEMO_MENU Menu_manage_next( uint8_t MenuId )
{
    if( MODE_SPY == hmode )
    {
        ...
        if( ( MENU_PD_SPEC == MenuId ) && ( 0 == DPM_Ports[0].DPM_NumberOfRcvSRCPDO ) )
        {
            return MENU_MEASURE;    // No PD config
        }

        if( ( MENU_PD_SPEC == MenuId ) && ( DPM_Ports[0].DPM_NumberOfRcvSRCPDO > 0 ) )
        {
            return MENU_POWER;    // PD CONFIG
        }

        if( MENU_POWER == MenuId )
        {
            return MENU_DUAL_ROLE_POWER;
        }

        if( MENU_DUAL_ROLE_POWER == MenuId )
        {
            return MENU_UNCHUNKED_MODE;
        }

        if( MENU_UNCHUNKED_MODE == MenuId )
        {
            return MENU_DATA_ROLE_SWAP;
        }

        if( MENU_DATA_ROLE_SWAP == MenuId )
        {
            return MENU_SELECT_SOURCECAPA;
        }

        if( MENU_SELECT_SOURCECAPA == MenuId )
        {
            return MENU_MEASURE;
        }

        if( MENU_MEASURE == MenuId )
        {
            return MENU_VERSION;
        }

        if( MENU_VERSION == MenuId )
        {
            return MENU_VERSION;
        }
    }
    else /* Standalone mode */
    {
        ...
        if( MENU_DATA_ROLE_SWAP == MenuId )
        {
            return MENU_DISPLAY_PORT;
        }

        if( MENU_DISPLAY_PORT == MenuId )
        {
            return MENU_MEASURE;
        }

        if( ( MENU_MEASURE == MenuId ) && ( DPM_Ports[0].DPM_NumberOfRcvSRCPDO > 0 ) )
        {
            return MENU_SELECT_SOURCECAPA;    // PD CONFIG
        }

        if( ( MENU_MEASURE == MenuId ) && ( 0 == DPM_Ports[0].DPM_NumberOfRcvSRCPDO ) )
        {
            return MENU_VERSION;    // no PD CONFIG
        }

        if( MENU_SELECT_SOURCECAPA == MenuId )
        {
            return MENU_SINKCAPA_RECEIVED;
        }

        if( MENU_SINKCAPA_RECEIVED == MenuId )
        {
            return MENU_COMMAND;
        }

        if( MENU_COMMAND == MenuId )
        {
            return MENU_VERSION;
        }
...
static DEMO_MENU Menu_manage_prev( uint8_t MenuId )
{
    if( MODE_SPY == hmode )
    {
        ...
        if( ( MENU_MEASURE == MenuId ) && ( DPM_Ports[0].DPM_NumberOfRcvSRCPDO > 0 ) )
        {
            return MENU_SELECT_SOURCECAPA;    // if PD
        }

        if( ( MENU_MEASURE == MenuId ) && ( 0 == DPM_Ports[0].DPM_NumberOfRcvSRCPDO ) )
        {
            return MENU_PD_SPEC;    // If not PD device
        }
    ...
    else  /* Standalone mode */
    {
        ...
        if( ( MENU_MEASURE == MenuId ) && ( DPM_Ports[0].DPM_NumberOfRcvSRCPDO > 0 ) )
        {
            return MENU_DISPLAY_PORT;    // If PD device
        }
        ...
        {
            if( ( MENU_VERSION == MenuId ) && ( 0 == DPM_Ports[0].DPM_NumberOfRcvSRCPDO ) )
            {
                return MENU_MEASURE;    // If not PD device
            }
        }

        if( ( MENU_VERSION == MenuId ) && ( DPM_Ports[0].DPM_NumberOfRcvSRCPDO > 0 ) )
        {
            return MENU_COMMAND;    // If PD device
        }
    }
...
static void Display_sourcecapa_menu( void )
{
    ...
    {
        if( DPM_Ports[0].DPM_ListOfRcvSRCPDO[0] & USBPD_PDO_SRC_FIXED_UNCHUNK_SUPPORTED )
        {
            sprintf( ( char * )_str, "%s UNCK", ( char * )_str );
        }

        if( DPM_Ports[0].DPM_ListOfRcvSRCPDO[0] & USBPD_PDO_SRC_FIXED_DRD_SUPPORT_Msk )
        {
            sprintf( ( char * )_str, "%s DRD", ( char * )_str );
        }

        if( DPM_Ports[0].DPM_ListOfRcvSRCPDO[0] & USBPD_PDO_SRC_FIXED_USBCOMM_NOT_SUPPORTED )
        {
            sprintf( ( char * )_str, "%s USB", ( char * )_str );
        }

        if( DPM_Ports[0].DPM_ListOfRcvSRCPDO[0] & USBPD_PDO_SRC_FIXED_DRP_SUPPORT_Msk )
        {
            sprintf( ( char * )_str, "%s DRP", ( char * )_str );
        }
    }
    ...
    if( ( DP_info_received == 1 ) && ( 1 == DPM_Ports[0].VDM_SVIDPortPartner.FlagAllSVIDReceived ) )
    {
        if( DPM_Ports[0].VDM_SVIDPortPartner.NumSVIDs > 0 )
        {
            ...
            else  /* if only one is supported : either thunderbolt, or display port */
            {
                ...
                if( 1 == DP_supported )
                {
                    sprintf( ( char * )_str, "DisplayPort" );
                }
                else if( 1 == Thunderbolt_supported )
                {
                    sprintf( ( char * )_str, "Thunderbolt" );
                }
                else
                {
                    sprintf( ( char * )_str, "Unknown VDM" );
                }
